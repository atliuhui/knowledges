"""Metadata editing service backing the MCP `kb.*_metadata_update` tools.
...
def _validate_patch(patch: dict[str, Any]) -> None:
    bad = set(patch.keys()) - md.HUMAN_FIELDS
    if bad:
        raise MetadataEditError(f"cannot modify tool-maintained fields: {sorted(bad)}")
    if "status" in patch and patch["status"] and patch["status"] not in md.ALLOWED_STATUSES:
        raise MetadataEditError(f"invalid status: {patch['status']}")
    if (
        "confidentiality" in patch
        and patch["confidentiality"] not in md.ALLOWED_CONFIDENTIALITIES
    ):
        raise MetadataEditError(f"invalid confidentiality: {patch['confidentiality']}")
...
def _diff(row: md.MetadataRow, patch: dict[str, Any]) -> dict[str, FieldChange]:
...
def _apply_bulk(row: md.MetadataRow, op: BulkOperation) -> dict[str, FieldChange]:
    ...
    if op.set_confidentiality is not None and op.set_confidentiality != row.confidentiality:
        if op.set_confidentiality not in md.ALLOWED_CONFIDENTIALITIES:
            raise MetadataEditError(f"invalid confidentiality: {op.set_confidentiality}")
    ...
    if op.set_status is not None and op.set_status != row.status:
        if op.set_status not in md.ALLOWED_STATUSES:
            raise MetadataEditError(f"invalid status: {op.set_status}")
...
def bulk_preview(cfg: Config, ids: Iterable[str], op: BulkOperation) -> dict[str, Any]:
    ...
    for row in rows:
        if row.id not in id_set:
            continue
        snapshot = md.MetadataRow(**row.__dict__)
